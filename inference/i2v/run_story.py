...
def get_clip(story_dir, clip, frame_path, seed):
    ...
    subprocess.run([sys.executable, os.path.join(BASE, "generate.py"),
                    "--image", frame_path, "--prefix", f"scene{clip['scene']:03d}",
                    "--frames", str(frames), "--seed", str(seed),
                    "--outdir", os.path.join(story_dir, "clips")], check=True)
    print(f"  [clip] {out}")
    return out
...
def main():
    ...
    for clip in clips:
        ...
        if not args.skip_video:
            ...
            clip_path = get_clip(story_dir, clip, frame if has_sdxl else None, args.seed + s)
