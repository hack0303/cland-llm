...
| **Von 可作为 TypeSafe 协议 drop-in** | `POST /v1/systemone`（noul/choice/score），单题 ~61ms、3 题 fan-out ~150ms，显存 ~1GB | 高（自测）+ 中（质量口径：经典 NLI sanity 正常，长语义先验题偏中性） |
...
| Von 单 noul / 3 题 fan-out | 61ms / 150ms | 显存 ~1.0GB，首请求懒加载 ~45s |
...
3. **Von 质量未经其 authored144 基准复验**：仅做经典 NLI sanity；其自评 91.23% 未在本机复现（基准集未随仓发布）。
