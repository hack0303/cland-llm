...
## 二之一、架构转向：全能管线 → 多管线分离（2026-08-16 决策）

> 背景：全能管线（单 AnimateDiff 同时承担角色+背景+动作）一路踩坑——脸/背景/动作互相妥协，效果不可控。
> 决策：**工业级 = 多管线各司其职 + 合成合并**，每个管线单目标、可调优、可替换。

```
┌─ 角色管线（白底）─────────────┐
│ 白底出图（脸部锚定）→ RMBG 透明 │  ← 锚定完美（同底色）
│ → 白底动画 → 透明视频序列       │
└───────────────────────────────┘
┌─ 背景管线（纯场景）─────────────┐
│ 纯场景出图（无角色）→ 背景图/视频 │  ← 背景质量拉满
└───────────────────────────────┘
        ↓ 合成管线（merge.py）
┌─ 合成管线 ─────────────────────┐
│ overlay 角色→背景 / 位置/缩放   │
...
**关键规则**：
1. 角色动画在**白底**上完成（与定妆同底色 → 锚定完美）
2. 合成在**动画后**（角色贴背景后再扩散会被重绘）
3. 每管线可独立调优/替换（角色换 LoRA、背景换模型、合成加滤镜）
4. 速度：白底 12s + 背景 12s + 动画 16帧 140s + 合成秒级
