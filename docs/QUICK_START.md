...
> ⚠️ 机器仅 15GB RAM：默认起 4 个服务（SDXL/TripoSG/TTS/音效），ASR 按需 `--with-asr`。机器重启后 /tmp 会清空，Spark-TTS 代码已内嵌项目，无需重新 clone。
...
| 内存不足（<1GB available） | 停 ASR：`kill $(pgrep -f "port 10334")`；必要时按需重启 |
